<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Partenaires</h2>
            <a href="{{ route('admin.partners.create') }}" class="px-4 py-2 rounded-md bg-gray-900 text-white hover:bg-gray-800">
                + Nouveau partenaire
            </a>
        </div>
    </x-slot>
            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <form method="GET" class="flex flex-col md:flex-row gap-3 md:items-end">
                    <div class="flex-1">
                        ...
                        <input name="q" value="{{ $q }}" class="mt-1 w-full border rounded-md p-2" placeholder="Nom, URL..." />
                    </div>
                    ...
                    <div>
                        ...
                        <select name="active" class="mt-1 w-full border rounded-md p-2">
                            ...
                            <option value="1" @selected($active === '1')>Oui</option>
                            <option value="0" @selected($active === '0')>Non</option>
                        </select>
                    </div>
                    ...
                    <tbody class="divide-y">
                        @foreach($partners as $p)
                            <tr>
                                ...
                                <td class="py-3 pr-4">
                                    @if($p->logo_path)
                                        <img src="{{ asset('storage/'.$p->logo_path) }}" class="h-8 w-auto" alt="{{ $p->name }}">
                                    @else
                                ...
                                <td class="py-3 pr-4">
                                    ...
                                    @if($p->website_url)
                                        <div class="text-gray-600 text-xs">{{ $p->website_url }}</div>
                                    @endif
                                </td>
                                <td class="py-3 pr-4">
                                ...
                                <td class="py-3 pr-4 flex items-center gap-4">
                                    <a href="{{ route('admin.partners.edit', $p) }}" class="text-gray-900 hover:underline">
                                        Éditer →
                                    </a>
